...
<script>
	...
	export let tooltipWidth = 150;

	export let keyColours = [];

	const w2 = tooltipWidth / 2;
...
<QuadTree dataset={dataset || $data} y="x" let:x let:y let:visible let:found let:e>
	...
	{#if visible === true}
		...
		>
			...
			{#if keyColours.length === 0}
				{#each foundSorted as row}
					{#if bold}
						<div class="py-0.5">
							<span class="key">{formatKey(row.key)}:</span> <b>{formatValue(row.value)}</b>{units}
						</div>
					{:else}
						<div class="py-0.5">
							<span class="key">{formatKey(row.key)}:</span>
							{formatValue(row.value)}{units}
						</div>
					{/if}
				{/each}
			{:else}
				{#each foundSorted as row}
					{#each keyColours as keyColour}
						{#if keyColour.key === row.key}
							{#if bold}
								<div
									class="py-0.5 my-1.5 border rounded-full px-1.5"
									style="border-color: {keyColour.hex}"
								>
									<span class="key">{formatKey(row.key)}:</span>
									<b>{formatValue(row.value)}</b>{units}
								</div>
							{:else}
								<div class="py-0.5">
									<span class="key">{formatKey(row.key)}:</span>
