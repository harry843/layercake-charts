	...
	>
		...
		<Html zIndex={2} pointerEvents={false}>
          ...
          {@const tooltipData = { ...detail.props }}
								...
								<div class="row p-0.5"><span class="font-medium">{addCommas(tooltipData.percentage)}%</span> have worked in {tooltipData.variable}</div>
								<div class="row p-0.5"><span class="font-medium">{addCommas(tooltipData.value)}</span> out of 310,352 survey participants </div>
	        </Tooltip>
			{/if}
		  </Html>
